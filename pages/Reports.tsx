...
const Reports: React.FC = () => {
  ...
  const plReport = useMemo(() => {
    const income = state.accounts.filter(a => a.type === 'Income').map(a => {
      ...
      // Income is credit-normal, so negative net balance means positive income
      return { title: a.title, amount: net < 0 ? Math.abs(net) : (net > 0 ? -net : 0) };
    });
    
    const expenses = state.accounts.filter(a => a.type === 'Expense').map(a => {
      const net = calculateAccountBalance(a.id, state);
      // Expense is debit-normal, so positive net balance means positive expense
      return { title: a.title, amount: net > 0 ? net : (net < 0 ? -Math.abs(net) : 0) };
    });
  ...
  const balanceSheet = useMemo(() => {
    ...
    // Re-calculate for individual parties if they cross over
    // But for a high-level balance sheet, we look at Account heads
    state.accounts.forEach(acc => {
      ...
      if (acc.code === '1003') {
        // Accounts Receivable: split into assets and liabilities based on individual party balances
        let arTotal = 0;
        let customerAdvances = 0;
        state.customers.forEach(c => {
          const cNet = calculateAccountBalance('acc-3', state, c.id);
          if (cNet > 0) arTotal += cNet;
          else customerAdvances += Math.abs(cNet);
        });
        if (arTotal > 0) assetsList.push({ title: 'Accounts Receivable (Trade)', amount: arTotal });
        if (customerAdvances > 0) liabilitiesList.push({ title: 'Customer Advances (Payable)', amount: customerAdvances });
      } 
      else if (acc.code === '2001') {
        // Accounts Payable: split into assets and liabilities
        let apTotal = 0;
        let vendorAdvances = 0;
        state.vendors.forEach(v => {
          const vNet = calculateAccountBalance('acc-5', state, v.id);
          if (vNet < 0) apTotal += Math.abs(vNet);
          else vendorAdvances += vNet;
        });
        if (apTotal > 0) liabilitiesList.push({ title: 'Accounts Payable (Trade)', amount: apTotal });
        if (vendorAdvances > 0) assetsList.push({ title: 'Vendor Prepayments (Advances)', amount: vendorAdvances });
      }
      else {
        // Standard non-control accounts
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
        ...
        <div className="space-y-6">
          <div className="bg-white dark:bg-slate-900 p-8 rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-sm grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="space-y-2">
              ...
              <select className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-xl px-4 py-3 text-sm font-bold outline-none dark:text-slate-200" value={glAccountId} onChange={e => setGlAccountId(e.target.value)}>
            ...
            <div className="space-y-2">
              ...
              <input type="date" className="w-full bg-slate-50 dark:bg-slate-800 border rounded-xl px-4 py-3 text-sm font-bold dark:text-slate-200" value={fromDate} onChange={e => setFromDate(e.target.value)} />
            </div>
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">To</label>
              <input type="date" className="w-full bg-slate-50 dark:bg-slate-800 border rounded-xl px-4 py-3 text-sm font-bold dark:text-slate-200" value={toDate} onChange={e => setToDate(e.target.value)} />
            </div>
          </div>
                ...
                <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
                  {glData.map((ent, i) => (
                    <tr key={i} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-all">
                      <td className="px-8 py-4 text-xs font-bold text-slate-500 dark:text-slate-400">{ent.date === 'Opening' ? '-' : ent.date}</td>
                      <td className="px-8 py-4 text-xs font-black text-sky-600">{ent.voucherNo}</td>
                      <td className="px-8 py-4 text-xs font-bold text-slate-600 dark:text-slate-300 truncate max-w-[200px]">{ent.description}</td>
                      <td className="px-8 py-4 text-right text-xs font-black dark:text-slate-200">{ent.debit > 0 ? ent.debit.toLocaleString() : '-'}</td>
                      <td className="px-8 py-4 text-right text-xs font-black dark:text-slate-200">{ent.credit > 0 ? ent.credit.toLocaleString() : '-'}</td>
                      <td className="px-8 py-4 text-right text-xs font-black dark:text-slate-100">
      ...
      {activeTab === 'Trial Balance' && (
        <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] overflow-hidden shadow-2xl border border-slate-100 dark:border-slate-800">
          ...
          <div className="divide-y divide-slate-50 dark:divide-slate-800">
            {trialBalance.map(acc => (
              <div key={acc.id} className="grid grid-cols-12 py-5 px-10 items-center">
                ...
                <div className="col-span-3 text-right text-xs font-black dark:text-slate-200">
          ...
          <div className="bg-[#0B1120] dark:bg-slate-800 py-8 px-10 flex justify-between items-center text-white">
            <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">System Totals</span>
      ...
      {activeTab === 'Profit & Loss' && (
        <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-12 shadow-2xl border border-slate-100 dark:border-slate-800">
           <div className="grid grid-cols-1 md:grid-cols-2 gap-16 mb-16">
             <div className="space-y-6">
                <p className="text-[10px] font-black text-emerald-500 uppercase tracking-widest border-b dark:border-slate-800 pb-4">Revenue Streams</p>
                {plReport.income.map((i, idx) => (
                  <div key={idx} className="flex justify-between items-center text-xs font-bold dark:text-slate-300">
                    <span>{i.title}</span><span>Rs. {i.amount.toLocaleString()}</span>
                  </div>
                ))}
             </div>
             <div className="space-y-6">
                <p className="text-[10px] font-black text-rose-400 uppercase tracking-widest border-b dark:border-slate-800 pb-4">Operational Overhead</p>
           ...
           <div className="bg-[#0B1120] rounded-[3rem] p-16 text-center text-white relative overflow-hidden shadow-2xl">
              <div className="relative z-10">
                 <p className="text-[10px] font-black text-sky-400 uppercase tracking-[0.3em] mb-4">Bottom Line Net Result</p>
                 <h2 className={`text-7xl font-black tracking-tighter ${plReport.netProfit >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                    Rs. {plReport.netProfit.toLocaleString()}
                 </h2>
              </div>
           </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-12 shadow-2xl border border-slate-100 dark:border-slate-800 space-y-12">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-20">
             <div className="space-y-8">
                <div className="bg-[#0B1120] text-white px-8 py-3 rounded-xl inline-block text-[10px] font-black uppercase tracking-widest mb-6">Non-Current & Current Assets</div>
                <div className="space-y-4">
                  {balanceSheet.assets.map((a, idx) => (
                    <div key={idx} className="flex justify-between items-center border-b dark:border-slate-800 pb-3 text-xs font-bold dark:text-slate-300">
                ...
                <div className="flex justify-between items-center pt-8 text-2xl font-black text-emerald-500">
                   <span className="text-[10px] uppercase">Total Statement Assets</span><span>Rs. {balanceSheet.totalAssets.toLocaleString()}</span>
                </div>
             </div>
             <div className="space-y-8">
                <div className="bg-[#0B1120] text-white px-8 py-3 rounded-xl inline-block text-[10px] font-black uppercase tracking-widest mb-6">Equity & Liabilities</div>
                <div className="space-y-4">
                  ...
                  <div className="flex justify-between items-center border-b dark:border-slate-800 pb-3 text-xs font-bold text-emerald-500">
                     <span>Retained Period Earnings (P&L)</span><span>Rs. {plReport.netProfit.toLocaleString()}</span>
                  </div>
                </div>
                <div className="flex justify-between items-center pt-8 text-2xl font-black text-rose-500">
                   <span className="text-[10px] uppercase">Total Equity & Liabilities</span><span>Rs. {(balanceSheet.totalLiabilities + plReport.netProfit).toLocaleString()}</span>
                </div>
             </div>
          </div>
          <div className="bg-slate-50 dark:bg-slate-800/50 p-6 rounded-2xl text-center text-[10px] font-bold text-slate-400 uppercase tracking-widest">
            Statement generated according to professional double-entry accounting standards (IFRS/GAAP)
          </div>
        </div>
      )}
    </div>
