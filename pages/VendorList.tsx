...
const VendorList: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    const vendorData: Partial<Vendor> = {
      id: editingId || undefined, code: vendorCode, name: formData.name, phone: formData.phone, email: formData.email, address: formData.address, city: formData.city, 
      openingBalance: formData.openingBalance, openingBalanceType: formData.side as any, isActive: formData.status === 'Active & Visible', status: formData.status as any
    };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex justify-between items-center">
        ...
        <button onClick={() => { setEditingId(null); setFormData({name: '', phone: '', email: '', city: '', address: '', status: 'Active & Visible', openingBalance: 0, side: 'Payable'}); setShowModal(true); }} className="bg-[#0B1120] text-white px-6 py-3 rounded-xl font-black text-xs uppercase tracking-widest shadow-lg hover:bg-slate-800 transition-all">
              ...
              {state.vendors.filter(v => v.name.toLowerCase().includes(searchTerm.toLowerCase())).map(v => {
                ...
                return (
                  <tr key={v.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
                    <td className="px-6 py-4 text-xs font-semibold text-sky-500">{v.code}</td>
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className={`text-sm font-black ${net < 0 ? 'text-rose-600' : net > 0 ? 'text-emerald-600' : 'text-slate-400'}`}>
                        {formatCurrency(Math.abs(net))}
                      </div>
                      <div className="text-[8px] font-black uppercase tracking-widest opacity-50">
                        {net <= 0 ? 'CR (Payable)' : 'DR (Advance Paid)'}
                      </div>
                    </td>
                    <td className="px-6 py-4 text-center">
                        <div className="flex items-center justify-center gap-2">
                          <button onClick={() => navigate(`/ledger/Vendor/${v.id}`)} className="text-slate-400 hover:text-sky-600 p-2"><i className="fa-solid fa-book"></i></button>
                          <button onClick={() => handleClone(v)} className="text-slate-400 hover:text-emerald-600 p-2"><i className="fa-solid fa-copy"></i></button>
                          <button onClick={() => handleEdit(v)} className="text-slate-400 hover:text-indigo-600 p-2"><i className="fa-solid fa-pen"></i></button>
                          <button onClick={() => deleteVendor(v.id)} className="text-slate-400 hover:text-rose-600 p-2"><i className="fa-solid fa-trash"></i></button>
                        </div>
                    </td>
                  </tr>
                );
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-[#0B1120]/60 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-3xl overflow-hidden shadow-2xl border border-slate-100 dark:border-slate-800 animate-in zoom-in-95 duration-200">
            <div className="bg-[#0B1120] text-white p-8 flex justify-between items-center">
              <div>
                <h2 className="font-black text-lg uppercase tracking-tighter">SUPPLIER REGISTRY</h2>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Commercial Partner Configuration</p>
              </div>
            ...
            <div className="p-8 space-y-6">
              <div className="space-y-1.5">
                ...
                <input 
                  className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-100 rounded-xl px-5 py-3.5 text-sm font-bold uppercase outline-none" 
                  value={formData.name} 
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <input 
                    className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-100 rounded-xl px-5 py-3.5 text-sm font-bold outline-none" 
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    >
                      ...
                      <option value="Advance">Advance Paid (Dr)</option>
                    </select>
                  </div>
                </div>
              </div>
